                ...
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-body">
                            ...
                            {!! Form::open(['url' => '/dashboard', 'method' => 'post', 'enctype' =>
                            'multipart/form-data']) !!}
                            <?php for ($i = 0; $i <= 3; $i++) { ?>
                                    ...
                                    <div class="col-lg-2 form-group">
                                        ...
                                        <select name="search_in[{{$i}}]" id="search_in_{{$i}}" data-id="{{$i}}"
                                            ...
                                            {{-- <option value="" @if(isset($inputValue) &&
                                                ($inputValue['search_in'][$i]=='' )){{{ "selected" }}} @endif></option>
                                            --}}
                                            ...
                                            <option value="zodiac_sign_id" @if(isset($inputValue) &&
                                                ...
                                                @endif>Zodiac Name</option>{{-- <option value="gender_flag"
                                                @if(isset($inputValue) && ($inputValue['search_in'][$i]=='gender_flag'
...
{     echo $inputValue['datefilter'][$i];} ?>" readonly
                                                required>
                                        </span>
                                    </div>
...
<button type="button"
            ...
            style="margin-right: 4px; font-weight: bold;"></i>Criteria
        </button>
            ...
            <div class="row">
                <div class="col-12">
                            ...
                            <table id="datatable-buttons"
                                ...
                                <thead>
                                    <tr class="text-center">
                                        ...
                                        <th class="text-center">Zodiac Sign</th>
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        {{-- <a href="#" id="viewUser" data-toggle="modal"
                                            data-target="#exampleModalCenter" data-id={{$item->user_id}}

                                            data-id="{{ $item->user_id }}" data-bs-toggle="modal"
                                            data-bs-target="#viewUserDetail">

                                            {{$item->full_name}}</a></td> --}}
                                        <td> <a href="#" class=" btn-sm edit view-user" title="View Profile"
                                                data-id="{{ $item->user_id }}"
                                                data-bs-toggle="modal"
                                                data-bs-target="view-user">{{$item->full_name}}</a> </td>

                                        <td>{{$item->phone_number}}</td>
                                        <td>{{$item->personal_pronounce}}</td>
                                        <td>{{$item->zodiac_sign_id}}</td>
                                      ...
                                      <td style="text-align: center;">
                                        <div class="dropdown">
                                            <button class="btn btn-light dropdown-toggle" type="button"
                                                id="dropdownMenu-{{ $item->user_id }}" data-bs-toggle="dropdown"
                                    ...
                                    <div class="dropdown-menu"
                                                aria-labelledby="dropdownMenu-{{ $item->user_id }}">
                    ...
                    <a id="delete-user" class="dropdown-item edit delete-user"
                                    title="Delete" data-id="{{ $item->user_id }}" data-bs-toggle="modal"
        ...
        $(document).on('click','.view-user-details', function(){
            ...
            if(id!=''){
                $.ajax({
                    type : 'GET',
                    data : {'id':id},
                    url  : "{{url('profile-view')}}",
                    success : function(data) {
                        $('.display-user-data-details').html(data);
                    }
                })
            }
        });
    ...
    <div class="modal fade bs-example-modal-center" id="viewUserDetails" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body display-user-data-details">
                </div>
                View Profile Modal
            </div><!-- /.modal-content -->
        </div><!-- /.modal-dialog -->
    ...
    {{--
    <script>
        $('#viewUserDetails').on('show.bs.modal', function (event) {
